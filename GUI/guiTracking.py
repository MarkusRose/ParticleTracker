...
class guiTracking(tk.Frame):
    ...
    def doSetup(self):
        ...
        tkinter.ttk.Button(self, text="Run", command=self.runTracking).grid(column=2,row=2, sticky="SE")
        ...
        tkinter.ttk.Button(self.labelframe, text="Positions File", command = lambda:self.inPosVar.set(tkinter.filedialog.askopenfilename())).grid(column=1, row=1, sticky='W')
        tkinter.ttk.Label(self.labelframe,textvariable=self.inPosVar).grid(column=3, row=1, sticky='W')

        
        self.maxDispVar = tk.StringVar()
        self.maxDispVar.set("3")
        tkinter.ttk.Label(self.labelframe, text="Maximum displacement").grid(column=1, row=2, sticky='W')
        tkinter.ttk.Entry(self.labelframe, textvariable = self.maxDispVar).grid(column=2, row=2, sticky='W')


        self.minTrVar = tk.StringVar()
        self.minTrVar.set('1')
        tkinter.ttk.Label(self.labelframe, text="Minimum track length").grid(column=1, row=3, sticky='W')
        tkinter.ttk.Entry(self.labelframe, textvariable = self.minTrVar).grid(column=2, row=3, sticky='W')

        self.linkRaVar = tk.StringVar()
        self.linkRaVar.set("2")
        tkinter.ttk.Label(self.labelframe, text="Link range (frameskip)").grid(column=1, row=4, sticky='W')
        tkinter.ttk.Entry(self.labelframe, textvariable = self.linkRaVar).grid(column=2, row=4, sticky='W')
    ...
    def checkInputs(self):
        try:
            if not os.path.isfile(self.inPosVar.get()):
                tkinter.messagebox.showerror("Not a file", "File does not exist. Please choose the correct file!")
                return False
            #Maximum displacement
            if float(self.maxDispVar.get()) <=0 :
                self.maxDispVar.set("2")
                tkinter.messagebox.showwarning("Reset Value", "Value(s) was/were incorrect. Reset to 2px.")
                return False

            #minimum track length
            if int(self.minTrVar.get())<1:
                self.minTrVar.set("1")
                tkinter.messagebox.showwarning("Reset Value", "Value(s) was/were incorrect. Reset to track length of 1.")
                return False

            #link range
            if int(self.linkRaVar.get())<1:
                self.minTrVar.set("1")
                tkinter.messagebox.showwarning("Reset Value", "Value(s) was/were incorrect. Link range set to 1.")
                return False

            #Initial Positions File
            #Output Folder
            if os.path.isdir(os.path.dirname(self.inPosVar.get())):
                if not tkinter.messagebox.askyesno("Folder Exists!", "This folder exists. Some content might be lost. Continue?"):
                    return False
        except ValueError:
    ...
    def runTracking(self):
        if self.checkInputs():
            ...
            def done_mssg():
                tkinter.messagebox.showinfo("Done!", "Tracking finished without problems.")
